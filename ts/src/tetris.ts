...
type Tetromino = 1 | 2 | 3 | 4 | 5 | 6 | 7;
let current = (Math.floor(Math.random() * 7) + 1) as Tetromino;
let y = 19;
let x = 3;
let rotation = 0;
let next = (Math.floor(Math.random() * 7) + 1) as Tetromino;
let lines = 0;
let level = 0;
let score = 0;
...
export function setupTetris(element: HTMLDivElement) {
  ...
  const canvas = element.querySelector<HTMLCanvasElement>("#playarea-canvas")!;
  ...
  const nextCanvas = element.querySelector<HTMLCanvasElement>("#next-canvas")!;
  ...
  // Get info elements
  const linesEl = element.querySelector<HTMLDivElement>("#lines-value")!;
  const levelEl = element.querySelector<HTMLDivElement>("#level-value")!;
  const scoreEl = element.querySelector<HTMLDivElement>("#score-value")!;
  ...
  // Track elapsed time
  let tick = baseSpeed;
  let lastTick: DOMHighResTimeStamp | undefined;
